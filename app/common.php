...
// 应用公共文件
use think\facade\Db;
// 安全修复：只在调试模式开启错误显示，生产环境关闭以避免信息泄露
// 检查 env 函数是否存在，避免在未完全初始化时出错
...
{
    $template = config_get('template', 'default');
    return app()->getRootPath() . config("view.view_dir_name") . '/index/' . $template . DIRECTORY_SEPARATOR;
}
...
function config_get($key, $default = null)
{
    ...
    // 如果配置不存在（命令行环境下），直接从数据库读取
    if ($value === null) {
        // 检查是否已安装，如果未安装则直接返回默认值，避免触发数据库连接错误
...
function verify_captcha4_slide(){
    if(!input('?post.captcha_id') || !input('?post.lot_number') || !input('?post.pass_token') || !input('?post.gen_time') || !input('?post.captcha_output')) {
        return '验证参数不完整';
    }
    $url = 'https://gcaptcha4.geetest.com/validate?captcha_id='.input('post.captcha_id');
    ...
    if(isset($arr['status']) && $arr['status']=='success'){
        if(isset($arr['result']) && $arr['result'] == 'success'){
            ...
            return '验证失败，'.(isset($arr['reason']) ? $arr['reason'] : '未知错误');
        }
    }else{
        return '验证失败，'.(isset($arr['msg']) ? $arr['msg'] : '请重新验证');
    }
}
